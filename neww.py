...
_WARM_CACHE_TTL = 300     # 5 minutes before re-warming a site

def _get_cached_warm_cookies(shop_url: str):
    """Return cached warming cookies for a site, or None if expired/missing."""
    with _WARM_CACHE_LOCK:
        entry = _WARM_CACHE.get(shop_url)
        if entry and (time.time() - entry["ts"]) < _WARM_CACHE_TTL:
...
def warm_storefront_session(session, shop_url, variant_id=None):
    ...
    Uses a per-site cache so the actual HTTP warming only happens once every 5 minutes.
    Subsequent calls for the same site just copy the cached cookies — near zero cost."""
    try:
        shop_url = normalize_shop_url(shop_url) if shop_url else shop_url
        
        # Fast path: reuse cached cookies from a previous warming for this site
        cached = _get_cached_warm_cookies(shop_url)
        if cached:
            for k, v in cached.items():
                session.cookies.set(k, v)
            print("  [WARM] Reusing cached storefront cookies (fast)")
            return
        
        fp = get_browser_fingerprint()
        
        # Step 1: Visit the homepage (sets _shopify_y, _shopify_s, _tracking_consent cookies)
        print("  [WARM] Visiting storefront (first time for this site)...")
        browse_headers = {
            'User-Agent': fp['user_agent'],
        ...
        try:
            r = session.get(shop_url, headers=browse_headers, timeout=8, verify=False, allow_redirects=True)
            print(f"  [WARM] Homepage: {r.status_code} ({len(r.cookies)} cookies set)")
        except Exception as e:
            print(f"  [WARM] Homepage visit failed (non-fatal): {e}")
        ...
        try:
            ...
            browse_headers['Referer'] = f"{shop_url}/"
            browse_headers['Sec-Fetch-Site'] = 'same-origin'
            r = session.get(collections_url, headers=browse_headers, timeout=8, verify=False, allow_redirects=True)
            print(f"  [WARM] Collections: {r.status_code}")
        except Exception as e:
            print(f"  [WARM] Collections visit failed (non-fatal): {e}")
        ...
        try:
            ...
            cart_headers = dict(browse_headers)
            cart_headers['Accept'] = 'application/json'
            cart_headers['Sec-Fetch-Dest'] = 'empty'
            cart_headers['Sec-Fetch-Mode'] = 'cors'
            cart_headers['X-Requested-With'] = 'XMLHttpRequest'
            r = session.get(cart_url, headers=cart_headers, timeout=6, verify=False)
            print(f"  [WARM] Cart init: {r.status_code}")
        except Exception:
            pass
        
        # Cache the cookies for reuse by other threads/cards on same site
        try:
            cookies_dict = {c.name: c.value for c in session.cookies}
            if cookies_dict:
                _store_warm_cookies(shop_url, cookies_dict)
        except Exception:
            pass
        
        print("  [WARM] Storefront warming complete")
        
    except Exception as e:
        # Non-fatal: proceed even if warming fails
        print(f"  [WARM] Storefront warming error (non-fatal): {e}")
...
def step1_add_to_cart(session):
    ...
    # Cookie warming: visit storefront first to build realistic session
    warm_storefront_session(session, SHOP_URL, VARIANT_ID)
...
def poll_for_delivery_and_expectations(session, checkout_token, session_token, merchandise_stable_id, max_attempts=7):
    ...
    for attempt in range(max_attempts):
        ...
        payload = add_persisted_query_ext(payload)
...
SHORT_SLEEP = 0.4  # Reduced for speed - relies on proxy rotation to avoid rate-limits
MAX_WAIT_SECONDS = 1.5  # Respect server pollDelay but cap aggressively
HTTP_TIMEOUT_SHORT = 8  # Fail-fast on dead proxies/connections
HTTP_TIMEOUT_MEDIUM = 12  # Fail-fast on slow responses
...
PARALLEL_THREADS = 12
PARALLEL_START_STAGGER_MIN_MS = 100
PARALLEL_START_STAGGER_MAX_MS = 300
...
def step1_add_to_cart_ctx(session, shop_url, variant_id, _429_retry_count=0):
    ...
    # Storefront warming removed for speed - cookies not required for checkout flow
    ...
    try:
        if _429_retry_count > 0:
            delay = 2.0 + (_429_retry_count * 1.5)
...
def poll_for_delivery_and_expectations_ctx(session, checkout_token, session_token, merchandise_stable_id, shop_url, variant_id, max_attempts=5):
    print(f"  [POLL] Waiting for delivery terms and expectations...")
    ...
    for attempt in range(max_attempts):
        ...
        if r.status_code == 429:
            wait = 2.0 + (attempt * 1.5) + random.uniform(0.5, 1.0)
            print(f"  [RATE-LIMITED] 429 on poll (attempt {attempt + 1}), waiting {wait:.1f}s...")
            time.sleep(wait)
            continue
        if r.status_code != 200:
            print(f"  [ERROR] HTTP {r.status_code}")
...
def poll_proposal_ctx(session, checkout_token, session_token, merchandise_stable_id, shipping_handle, shop_url, variant_id, phone_required=False, shipping_amount=None, max_attempts=5):
    ...
    for attempt in range(max_attempts):
        ...
        if r.status_code == 429:
            wait = 2.0 + (attempt * 1.5) + random.uniform(0.5, 1.0)
            print(f"  [RATE-LIMITED] 429 on expectation poll (attempt {attempt + 1}), waiting {wait:.1f}s...")
            time.sleep(wait)
            continue
        if r.status_code == 200:
            try:
...
def step3_proposal_ctx(session, checkout_token, session_token, card_session_id, shop_url, variant_id):
    ...
    # Retry proposal on 429 with backoff (rate limiting is common with concurrent requests)
    response = None
    for proposal_attempt in range(3):
        r = session.post(url, json=payload, headers=headers, timeout=HTTP_TIMEOUT_SHORT, verify=False)
        if r.status_code == 429:
            wait = 2.0 + (proposal_attempt * 2.0) + random.uniform(0.5, 1.5)
            print(f"  [RATE-LIMITED] 429 on proposal (attempt {proposal_attempt + 1}/3), waiting {wait:.1f}s...")
            time.sleep(wait)
            continue
        break
    else:
        print(f"  [ERROR] Proposal failed after 3 attempts (429 rate limited)")
        return None, None, None, None, None, False
