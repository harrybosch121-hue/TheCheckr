...
async def cmd_txt_with_load_balancing(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with _active_requests_lock:
        if user_id and user_id not in _active_requests and len(_active_requests) >= _max_concurrent_requests:
            is_busy = True
        if user_id and not is_busy:
            _active_requests[user_id] = request_start_time

    if is_busy:
        await update.message.reply_text(
            "⏳ <b>System Busy</b>\n\nToo many batch requests are starting at once. Please try again in a moment.",
            parse_mode=ParseMode.HTML
        )
        return
