...
def _prepare_runtime_data_dir() -> None:
    ...
    On Railway, mount a volume and set `DATA_DIR=/data`.
    All relative runtime files will then live on that volume.
    """
    if not DATA_DIR:
        return
    ...
    try:
        ...
        os.makedirs(os.path.join(DATA_DIR, "ng"), exist_ok=True)
        ...
        seed_files = [
            ...
            "approved.txt",
            "bot_config.txt",
            os.path.join("ng", "user_proxies.txt"),
        ]
        ...
        print(f"[DATA_DIR] Failed to initialize runtime data dir: {e}")
...
BOT_TOKEN = "8214863704:AAFbWNL3I0Cef01qqbSMSkeG1TLGP6BYveg"
...
STATS_FLUSH_MIN_SECONDS = 5.0
# Dynamic batch workers based on batch size AND site count
def get_batch_workers(batch_size: int, num_sites: int = 1) -> int:
    """Calculate optimal batch workers based on batch size and available sites.
    Key insight: with N sites, max ~3 concurrent workers per site to avoid 429 rate limits."""
    # Hard cap: max 3 concurrent workers per available site
    site_cap = max(1, num_sites) * 3
    
    if batch_size < 100:
        ideal = min(20, batch_size)
    ...
    # The site cap is the real constraint - never exceed it
    return min(ideal, site_cap)
BATCH_WORKERS = 100  # Default for backwards compatibility
BROADCAST_WORKERS = 20

UPLOADS_DIR = "uploads"
STREAM_BATCH_PREFIX = "stream_batch"
...
def _make_stream_batch_file_path(user_id: int) -> str:
    ensure_uploads_dir()
    return os.path.join(UPLOADS_DIR, f"{STREAM_BATCH_PREFIX}_{user_id}_{time.time_ns()}.txt")

def _write_stream_batch_file(source_path: str, stream_path: str) -> int:
    """Parse a txt file line-by-line and write normalized cards to a disk-backed batch file."""
    total = 0
    try:
        with open(source_path, "r", encoding="utf-8", errors="ignore") as src, open(stream_path, "w", encoding="utf-8") as dst:
            for line in src:
                try:
                ...
                month = str(card.get("month", "")).strip()
                year = str(card.get("year", "")).strip()
                cvv = str(card.get("verification_value", "")).strip()
                name = str(card.get("name", "Test Card") or "Test Card").replace("|", " ").strip()
                if not (number and month and year and cvv):
                    continue
                dst.write(f"{number}|{month}|{year}|{cvv}|{name}\n")
                total += 1
    except Exception:
        total = 0
...
def _iter_stream_batch_cards(stream_path: str, skip: int = 0):
    skipped = 0
    with open(stream_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            if len(parts) < 4:
                continue
            number, month, year, cvv = parts[:4]
            name = parts[4] if len(parts) > 4 and parts[4].strip() else "Test Card"
            yield {
                "number": number.strip(),
                "month": month.strip(),
                "year": year.strip(),
                "verification_value": cvv.strip(),
                "name": name.strip() or "Test Card",
            }
...
HARDCODED_ADMIN_IDS = [8194958384, 5733576801]
...
GLOBAL_CHECKOUT_SEMAPHORE = asyncio.Semaphore(GLOBAL_CHECKOUT_LIMIT)
...
async def get_active_batch_count() -> int:
    """Return the number of currently active, non-paused batches."""
    try:
        async with ACTIVE_LOCK:
            return sum(
                1
                for rec in ACTIVE_BATCHES.values()
                if isinstance(rec, dict) and not rec.get("paused_for_proxies")
            )
    except Exception:
        return 0

async def is_system_batch_capacity_reached() -> bool:
    try:
        return await get_active_batch_count() >= GLOBAL_ACTIVE_BATCH_LIMIT
    except Exception:
        return False
...
async def resume_pending_batches(app):
    ...
    for batch_id, payload in pend.items():
        try:
            ...
            card_file_path = payload.get("card_file") or None
            ...
            if (not isinstance(cards, list) and not card_file_path) or not isinstance(sites, list) or not chat_id or not user_id:
                continue
            ...
            processed = 0
            original_total = int(payload.get("total", 0) or 0)
            if original_total <= 0:
            ...
            if card_file_path and not os.path.exists(card_file_path):
                logger.warning(f"Skipping resume for batch {batch_id} - card file missing: {card_file_path}")
                await remove_pending(batch_id)
                continue
            ...
            try:
                batch_size_for_executor = original_total if original_total > 0 else (len(cards) if isinstance(cards, list) else 0)
                chosen_executor = GLOBAL_EXECUTOR if batch_size_for_executor > SMALL_BATCH_THRESHOLD else SMALL_TASK_EXECUTOR
            except Exception:
            ...
            runner = BatchRunner(cards, sites, chosen_executor, resumed_batch_id, chat_id, user_id, cancel_event, 
                            send_approved_notifications=send_approved, proxies_override=proxy_mapping, start_from=processed, total_override=original_total, captcha_mode=captcha_mode, card_file_path=card_file_path)
...
def check_single_card(card: Dict, sites: List[str], proxies_override: Optional[Dict[str, str]] = None, runner=None) -> Tuple[str, str, str, str, Optional[str], Optional[str], Optional[str]]:
    ...
    # IMPORTANT: Make a defensive copy of sites to avoid concurrency issues
    # (other threads may mutate the original list via sites[:] = ...)
    sites_snapshot = list(sites) if sites else []
    filtered_sites = []
    if isinstance(sites_snapshot, list):
        ...
        filtered_sites = list(sites_snapshot) if sites_snapshot else []
    ...
    for site_idx, site in enumerate(ordered_sites):
        ...
        if site_idx > 0:
            # Brief delay between sites to prevent rate limiting
            try:
                if runner and hasattr(runner, 'cancel_event') and runner.cancel_event.is_set():
                    return ("unknown", '"code": "CANCELLED"', "$0", "", used_proxy_url, None, None)
            except Exception:
        ...
        try:
                    ...
                    try:
                        ...
                        if test_response.status_code == 200:
                            ...
                            try:
                                ...
                                print(f"[DEBUG] Product detection - Site removed from local sites list (404)")
                            except Exception as e:
            ...
            if not queue_token or not shipping_handle:
                ...
                # Remove from local ordered_sites only (not the shared sites list)
                # This prevents a concurrency bug where one thread empties the shared
                # list for ALL other concurrent threads, causing mass UNKNOWN results
                try:
                    normalized_target = checkout.normalize_shop_url(shop_url).rstrip("/")
                    ordered_sites = [s for s in ordered_sites if checkout.normalize_shop_url(s).rstrip("/") != normalized_target]
                except Exception:
            ...
            if not receipt_id:
                ...
                if is_site_level_error or no_receipt_id_site_error:
                    ...
                    try:
                        normalized_target = checkout.normalize_shop_url(shop_url).rstrip("/")
                        ordered_sites = [s for s in ordered_sites if checkout.normalize_shop_url(s).rstrip("/") != normalized_target]
                        print(f"[DEBUG] Step 4 - Site removed from local sites list")
                    except Exception as e:
...
class BatchRunner:
    def __init__(self, cards: List[Dict], sites: List[str], executor: ThreadPoolExecutor, batch_id: str, chat_id: int, user_id: int, cancel_event: asyncio.Event, send_approved_notifications: bool = True, proxies_override: Optional[Dict[str, str]] = None, start_from: int = 0, total_override: Optional[int] = None, captcha_mode: str = "WITH_RETRY", card_file_path: Optional[str] = None):
        ...
        self.unknown_retry_max = 3  # With few sites, retrying just adds more 429 pressure
        self.unknown_retry_delay = 2.0  # Give site time to recover between retries
    ...
    def _iter_source_cards(self):
        if self.card_file_path:
            yield from _iter_stream_batch_cards(self.card_file_path, skip=self._source_skip)
            return
    ...
    def _build_pending_payload(self, title: str) -> Dict:
        payload = {
        ...
        if self.card_file_path:
            payload["card_file"] = self.card_file_path
        else:
            payload["cards"] = self.cards
        return payload

    def _cleanup_stream_file(self):
    ...
    async def _flush_stats(self, update: Update):
        try:
            pending = dict(self._stats_pending)
            if pending.get("tested", 0) <= 0:
                return
            self._stats_pending = {"tested": 0, "approved": 0, "charged": 0}
            self._stats_last_flush = time.time()
            ...
            await inc_user_stats(
                ...
                tested=pending.get("tested", 0),
                approved=pending.get("approved", 0),
                charged=pending.get("charged", 0),
                chat_id=update.effective_chat.id,
            )
        except Exception:
            pass

    async def _queue_stats_update(self, update: Update, status: str):
        try:
            self._stats_pending["tested"] += 1
            if status == "approved":
                self._stats_pending["approved"] += 1
            elif status == "charged":
                self._stats_pending["charged"] += 1

            now = time.time()
            if (
                self._stats_pending["tested"] >= STATS_FLUSH_MIN_CARDS
                or (now - self._stats_last_flush) >= STATS_FLUSH_MIN_SECONDS
            ):
    ...
    async def _save_to_pending_immediately(self, title: str = "Batch"):
        ...
        try:
            async with self._pending_save_lock:
                ...
                await add_pending(self.batch_id, self._build_pending_payload(title))
    ...
    async def run_with_notifications(self, update: Update, context: ContextTypes.DEFAULT_TYPE, title: str):
        ...
        # Use dynamic batch workers based on batch size AND available sites
        num_sites = len(self.sites) if self.sites else 1
        optimal_workers = get_batch_workers(self.total, num_sites)
        sem = asyncio.Semaphore(optimal_workers)
        logger.info(f"[BATCH] Using {optimal_workers} concurrent workers for batch of {self.total} cards across {num_sites} sites")
            ...
            async with sem:
                ...
                while True:
                    ...
                    try:
                        ...
                        async with GLOBAL_CHECKOUT_SEMAPHORE:
        ...
        async def produce_cards():
            try:
                produced = 0
                for card in self._iter_source_cards():
                    if self.cancel_event.is_set() or self.paused_for_proxies:
                        break
                ...
                logger.error(f"Error producing cards for batch {self.batch_id}: {e}")
            finally:
                for _ in range(worker_count):
                    await card_queue.put(worker_done_marker)

        async def worker_loop():
            while True:
                card = await card_queue.get()
                if card is worker_done_marker:
                    await result_queue.put(worker_done_marker)
                    return
                try:
                    result = await run_one(card)
                except Exception:
                    result = (card, "unknown", '"code": "UNKNOWN"', "$0", "", None, None, None)
                await result_queue.put(result)

        producer_task = asyncio.create_task(produce_cards())
        worker_tasks = [asyncio.create_task(worker_loop()) for _ in range(worker_count)]
        ...
        while completed_workers < worker_count:
            ...
            if item is worker_done_marker:
                completed_workers += 1
                continue

            card, status, code_display, amount_display, site_label, used_proxy_url, site_url, receipt_id = item
            ...
            async with self.lock:
                ...
                await self._queue_stats_update(update, status)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "👨‍💻 <b>Owner:</b> @aldorsi\n"
        "📢 <b>Join our GC:</b> https://t.me/+89lKTv0c4zNhOWY0",
        parse_mode=ParseMode.HTML
    )
...
async def cmd_txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Build a normalized disk-backed card stream file and load sites in parallel
    loop = asyncio.get_event_loop()
    stream_path = _make_stream_batch_file_path(user.id)
    cards_task = loop.run_in_executor(IO_EXECUTOR, _write_stream_batch_file, txt_path, stream_path)
    sites_task = loop.run_in_executor(IO_EXECUTOR, _get_cached_sites)
    
    # Wait for both to complete
    total_cards, sites = await asyncio.gather(cards_task, sites_task)
    ...
    context.chat_data["pending_card_file"] = stream_path
    context.chat_data["pending_card_count"] = int(total_cards)
...
async def cmd_mass(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await ensure_access(update, context):
        return
    if (not is_admin(update.effective_user.id)) and await is_system_batch_capacity_reached():
        await update.message.reply_text(
            f"⏳ System is busy right now. Please wait for a few checks to finish.\n\n"
            f"Active batches limit: {GLOBAL_ACTIVE_BATCH_LIMIT}",
            parse_mode=ParseMode.HTML
        )
        return
    full_text = (update.message.text or "").strip()
...
async def cmd_sh(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user = update.effective_user
    if (not is_admin(user.id)) and await is_system_batch_capacity_reached():
        await update.message.reply_text(
            f"⏳ System is busy right now. Please wait for a few checks to finish.\n\n"
            f"Active batches limit: {GLOBAL_ACTIVE_BATCH_LIMIT}",
            parse_mode=ParseMode.HTML
...
async def pref_approved_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    card_file_path = context.chat_data.get("pending_card_file") or None
    pending_card_count = int(context.chat_data.get("pending_card_count") or 0)
    ...
    if not cards and not card_file_path:
        txt_path = context.chat_data.get("last_txt_path")
        if not (txt_path and os.path.exists(txt_path)):
            ...
            return
        card_file_path = _make_stream_batch_file_path(update.effective_user.id)
        loop = asyncio.get_event_loop()
        pending_card_count = await loop.run_in_executor(IO_EXECUTOR, _write_stream_batch_file, txt_path, card_file_path)

    if card_file_path and (not os.path.exists(card_file_path)):
        await update.effective_chat.send_message("Pending card file is missing. Please send a .txt and use /txt again.")
        return

    if not card_file_path and not cards:
        await update.effective_chat.send_message("No valid cards found in the pending batch.")
        return
    ...
    try:
        ...
        proxy_mapping = None
    effective_total = pending_card_count if card_file_path else (len(cards) if isinstance(cards, list) else 0)
    try:
        chosen_executor = GLOBAL_EXECUTOR if effective_total > SMALL_BATCH_THRESHOLD else SMALL_TASK_EXECUTOR
    except Exception:
        chosen_executor = GLOBAL_EXECUTOR
    runner = BatchRunner(cards, sites, chosen_executor, batch_id, update.effective_chat.id, update.effective_user.id, cancel_event, send_approved_notifications=send_approved, proxies_override=proxy_mapping, captcha_mode=captcha_mode, total_override=effective_total, card_file_path=card_file_path)
    try:
        async with ACTIVE_LOCK:
            ACTIVE_BATCHES[batch_id] = {
                "event": cancel_event,
        ...
        pass
    try:
        await add_pending(batch_id, runner._build_pending_payload(title))
    except Exception:
        pass
    ...
    try:
        context.chat_data.pop("pending_cards", None)
        context.chat_data.pop("pending_card_file", None)
        context.chat_data.pop("pending_card_count", None)
...
async def run_scraper_task(user_id, chat_id, num_sites, user_proxies, cancel_event, progress_msg, scraper_id):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=min(5, len(user_proxies))) as executor:
